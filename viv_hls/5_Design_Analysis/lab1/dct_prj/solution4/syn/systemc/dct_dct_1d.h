// ==============================================================
// RTL generated by Vivado(TM) HLS - High-Level Synthesis from C, C++ and SystemC
// Version: 2015.3
// Copyright (C) 2015 Xilinx Inc. All rights reserved.
// 
// ===========================================================

#ifndef _dct_dct_1d_HH_
#define _dct_dct_1d_HH_

#include "systemc.h"
#include "AESL_pkg.h"

#include "dct_mac_muladd_16s_15s_29s_29_1.h"
#include "dct_mac_muladd_16s_15s_14ns_29_1.h"
#include "dct_mul_mul_16s_15s_29_1.h"
#include "dct_mac_muladd_16s_14ns_29s_29_1.h"
#include "dct_dct_1d_dct_coeff_table_0.h"
#include "dct_dct_1d_dct_coeff_table_1.h"
#include "dct_dct_1d_dct_coeff_table_2.h"
#include "dct_dct_1d_dct_coeff_table_3.h"
#include "dct_dct_1d_dct_coeff_table_4.h"
#include "dct_dct_1d_dct_coeff_table_5.h"
#include "dct_dct_1d_dct_coeff_table_6.h"
#include "dct_dct_1d_dct_coeff_table_7.h"

namespace ap_rtl {

struct dct_dct_1d : public sc_module {
    // Port declarations 36
    sc_in_clk ap_clk;
    sc_in< sc_logic > ap_rst;
    sc_in< sc_logic > ap_start;
    sc_out< sc_logic > ap_done;
    sc_out< sc_logic > ap_idle;
    sc_out< sc_logic > ap_ready;
    sc_out< sc_lv<3> > src_address0;
    sc_out< sc_logic > src_ce0;
    sc_in< sc_lv<16> > src_q0;
    sc_out< sc_lv<3> > src1_address0;
    sc_out< sc_logic > src1_ce0;
    sc_in< sc_lv<16> > src1_q0;
    sc_out< sc_lv<3> > src2_address0;
    sc_out< sc_logic > src2_ce0;
    sc_in< sc_lv<16> > src2_q0;
    sc_out< sc_lv<3> > src3_address0;
    sc_out< sc_logic > src3_ce0;
    sc_in< sc_lv<16> > src3_q0;
    sc_out< sc_lv<3> > src4_address0;
    sc_out< sc_logic > src4_ce0;
    sc_in< sc_lv<16> > src4_q0;
    sc_out< sc_lv<3> > src5_address0;
    sc_out< sc_logic > src5_ce0;
    sc_in< sc_lv<16> > src5_q0;
    sc_out< sc_lv<3> > src6_address0;
    sc_out< sc_logic > src6_ce0;
    sc_in< sc_lv<16> > src6_q0;
    sc_out< sc_lv<3> > src7_address0;
    sc_out< sc_logic > src7_ce0;
    sc_in< sc_lv<16> > src7_q0;
    sc_in< sc_lv<4> > tmp_2;
    sc_out< sc_lv<6> > dst_address0;
    sc_out< sc_logic > dst_ce0;
    sc_out< sc_logic > dst_we0;
    sc_out< sc_lv<16> > dst_d0;
    sc_in< sc_lv<4> > tmp_21;


    // Module declarations
    dct_dct_1d(sc_module_name name);
    SC_HAS_PROCESS(dct_dct_1d);

    ~dct_dct_1d();

    sc_trace_file* mVcdFile;

    dct_dct_1d_dct_coeff_table_0* dct_coeff_table_0_U;
    dct_dct_1d_dct_coeff_table_1* dct_coeff_table_1_U;
    dct_dct_1d_dct_coeff_table_2* dct_coeff_table_2_U;
    dct_dct_1d_dct_coeff_table_3* dct_coeff_table_3_U;
    dct_dct_1d_dct_coeff_table_4* dct_coeff_table_4_U;
    dct_dct_1d_dct_coeff_table_5* dct_coeff_table_5_U;
    dct_dct_1d_dct_coeff_table_6* dct_coeff_table_6_U;
    dct_dct_1d_dct_coeff_table_7* dct_coeff_table_7_U;
    dct_mac_muladd_16s_15s_29s_29_1<1,1,16,15,29,29>* dct_mac_muladd_16s_15s_29s_29_1_U10;
    dct_mac_muladd_16s_15s_29s_29_1<1,1,16,15,29,29>* dct_mac_muladd_16s_15s_29s_29_1_U11;
    dct_mac_muladd_16s_15s_29s_29_1<1,1,16,15,29,29>* dct_mac_muladd_16s_15s_29s_29_1_U12;
    dct_mac_muladd_16s_15s_14ns_29_1<1,1,16,15,14,29>* dct_mac_muladd_16s_15s_14ns_29_1_U13;
    dct_mul_mul_16s_15s_29_1<1,1,16,15,29>* dct_mul_mul_16s_15s_29_1_U14;
    dct_mul_mul_16s_15s_29_1<1,1,16,15,29>* dct_mul_mul_16s_15s_29_1_U15;
    dct_mul_mul_16s_15s_29_1<1,1,16,15,29>* dct_mul_mul_16s_15s_29_1_U16;
    dct_mac_muladd_16s_14ns_29s_29_1<1,1,16,14,29,29>* dct_mac_muladd_16s_14ns_29s_29_1_U17;
    sc_signal< sc_lv<3> > ap_CS_fsm;
    sc_signal< sc_logic > ap_sig_cseq_ST_st1_fsm_0;
    sc_signal< bool > ap_sig_bdd_21;
    sc_signal< sc_lv<3> > dct_coeff_table_0_address0;
    sc_signal< sc_logic > dct_coeff_table_0_ce0;
    sc_signal< sc_lv<14> > dct_coeff_table_0_q0;
    sc_signal< sc_lv<3> > dct_coeff_table_1_address0;
    sc_signal< sc_logic > dct_coeff_table_1_ce0;
    sc_signal< sc_lv<15> > dct_coeff_table_1_q0;
    sc_signal< sc_lv<3> > dct_coeff_table_2_address0;
    sc_signal< sc_logic > dct_coeff_table_2_ce0;
    sc_signal< sc_lv<15> > dct_coeff_table_2_q0;
    sc_signal< sc_lv<3> > dct_coeff_table_3_address0;
    sc_signal< sc_logic > dct_coeff_table_3_ce0;
    sc_signal< sc_lv<15> > dct_coeff_table_3_q0;
    sc_signal< sc_lv<3> > dct_coeff_table_4_address0;
    sc_signal< sc_logic > dct_coeff_table_4_ce0;
    sc_signal< sc_lv<15> > dct_coeff_table_4_q0;
    sc_signal< sc_lv<3> > dct_coeff_table_5_address0;
    sc_signal< sc_logic > dct_coeff_table_5_ce0;
    sc_signal< sc_lv<15> > dct_coeff_table_5_q0;
    sc_signal< sc_lv<3> > dct_coeff_table_6_address0;
    sc_signal< sc_logic > dct_coeff_table_6_ce0;
    sc_signal< sc_lv<15> > dct_coeff_table_6_q0;
    sc_signal< sc_lv<3> > dct_coeff_table_7_address0;
    sc_signal< sc_logic > dct_coeff_table_7_ce0;
    sc_signal< sc_lv<15> > dct_coeff_table_7_q0;
    sc_signal< sc_lv<4> > k_reg_288;
    sc_signal< sc_lv<8> > tmp_16_cast_fu_307_p1;
    sc_signal< sc_lv<8> > tmp_16_cast_reg_490;
    sc_signal< sc_lv<3> > src_addr_reg_495;
    sc_signal< sc_lv<3> > src1_addr_reg_500;
    sc_signal< sc_lv<3> > src2_addr_reg_505;
    sc_signal< sc_lv<3> > src3_addr_reg_510;
    sc_signal< sc_lv<3> > src4_addr_reg_515;
    sc_signal< sc_lv<3> > src5_addr_reg_520;
    sc_signal< sc_lv<3> > src6_addr_reg_525;
    sc_signal< sc_lv<3> > src7_addr_reg_530;
    sc_signal< sc_lv<1> > exitcond1_fu_323_p2;
    sc_signal< sc_lv<1> > exitcond1_reg_535;
    sc_signal< sc_logic > ap_sig_cseq_ST_pp0_stg0_fsm_1;
    sc_signal< bool > ap_sig_bdd_169;
    sc_signal< sc_logic > ap_reg_ppiten_pp0_it0;
    sc_signal< sc_logic > ap_reg_ppiten_pp0_it1;
    sc_signal< sc_logic > ap_reg_ppiten_pp0_it2;
    sc_signal< sc_logic > ap_reg_ppiten_pp0_it3;
    sc_signal< sc_logic > ap_reg_ppiten_pp0_it4;
    sc_signal< sc_lv<1> > ap_reg_ppstg_exitcond1_reg_535_pp0_it1;
    sc_signal< sc_lv<1> > ap_reg_ppstg_exitcond1_reg_535_pp0_it2;
    sc_signal< sc_lv<1> > ap_reg_ppstg_exitcond1_reg_535_pp0_it3;
    sc_signal< sc_lv<4> > k_1_fu_329_p2;
    sc_signal< sc_lv<8> > tmp_5_fu_351_p2;
    sc_signal< sc_lv<8> > tmp_5_reg_544;
    sc_signal< sc_lv<8> > ap_reg_ppstg_tmp_5_reg_544_pp0_it1;
    sc_signal< sc_lv<8> > ap_reg_ppstg_tmp_5_reg_544_pp0_it2;
    sc_signal< sc_lv<8> > ap_reg_ppstg_tmp_5_reg_544_pp0_it3;
    sc_signal< sc_lv<14> > dct_coeff_table_0_load_reg_589;
    sc_signal< sc_lv<14> > ap_reg_ppstg_dct_coeff_table_0_load_reg_589_pp0_it2;
    sc_signal< sc_lv<15> > dct_coeff_table_1_load_reg_594;
    sc_signal< sc_lv<16> > src1_load_reg_599;
    sc_signal< sc_lv<15> > dct_coeff_table_2_load_reg_604;
    sc_signal< sc_lv<15> > ap_reg_ppstg_dct_coeff_table_2_load_reg_604_pp0_it2;
    sc_signal< sc_lv<15> > dct_coeff_table_3_load_reg_609;
    sc_signal< sc_lv<16> > src3_load_reg_614;
    sc_signal< sc_lv<15> > dct_coeff_table_4_load_reg_619;
    sc_signal< sc_lv<15> > ap_reg_ppstg_dct_coeff_table_4_load_reg_619_pp0_it2;
    sc_signal< sc_lv<15> > dct_coeff_table_5_load_reg_624;
    sc_signal< sc_lv<16> > src5_load_reg_629;
    sc_signal< sc_lv<15> > dct_coeff_table_6_load_reg_634;
    sc_signal< sc_lv<16> > src6_load_reg_639;
    sc_signal< sc_lv<15> > dct_coeff_table_7_load_reg_644;
    sc_signal< sc_lv<16> > src7_load_reg_649;
    sc_signal< sc_lv<16> > src_load_reg_654;
    sc_signal< sc_lv<29> > tmp_10_1_fu_464_p2;
    sc_signal< sc_lv<29> > tmp_10_1_reg_659;
    sc_signal< sc_lv<16> > src2_load_reg_664;
    sc_signal< sc_lv<29> > tmp_10_3_fu_476_p2;
    sc_signal< sc_lv<29> > tmp_10_3_reg_669;
    sc_signal< sc_lv<16> > src4_load_reg_674;
    sc_signal< sc_lv<29> > tmp_10_5_fu_470_p2;
    sc_signal< sc_lv<29> > tmp_10_5_reg_679;
    sc_signal< sc_lv<29> > grp_fu_448_p3;
    sc_signal< sc_lv<29> > tmp6_reg_684;
    sc_signal< sc_lv<16> > tmp_7_reg_689;
    sc_signal< sc_lv<64> > tmp_2_cast_fu_311_p1;
    sc_signal< sc_lv<64> > tmp_fu_335_p1;
    sc_signal< sc_lv<64> > tmp_17_cast_fu_428_p1;
    sc_signal< sc_lv<7> > tmp_4_fu_299_p3;
    sc_signal< sc_lv<8> > tmp_cast_fu_347_p1;
    sc_signal< sc_lv<29> > grp_fu_482_p3;
    sc_signal< sc_lv<29> > grp_fu_432_p3;
    sc_signal< sc_lv<29> > grp_fu_440_p3;
    sc_signal< sc_lv<29> > tmp1_fu_404_p2;
    sc_signal< sc_lv<29> > tmp4_fu_408_p2;
    sc_signal< sc_lv<29> > tmp_s_fu_412_p2;
    sc_signal< sc_lv<29> > grp_fu_455_p3;
    sc_signal< sc_lv<14> > grp_fu_455_p2;
    sc_signal< sc_lv<14> > grp_fu_482_p1;
    sc_signal< sc_logic > ap_sig_cseq_ST_st7_fsm_2;
    sc_signal< bool > ap_sig_bdd_403;
    sc_signal< sc_lv<3> > ap_NS_fsm;
    sc_signal< sc_lv<29> > grp_fu_482_p10;
    static const sc_logic ap_const_logic_1;
    static const sc_logic ap_const_logic_0;
    static const sc_lv<3> ap_ST_st1_fsm_0;
    static const sc_lv<3> ap_ST_pp0_stg0_fsm_1;
    static const sc_lv<3> ap_ST_st7_fsm_2;
    static const bool ap_true;
    static const sc_lv<32> ap_const_lv32_0;
    static const sc_lv<1> ap_const_lv1_1;
    static const sc_lv<32> ap_const_lv32_1;
    static const sc_lv<1> ap_const_lv1_0;
    static const sc_lv<4> ap_const_lv4_0;
    static const sc_lv<3> ap_const_lv3_0;
    static const sc_lv<4> ap_const_lv4_8;
    static const sc_lv<4> ap_const_lv4_1;
    static const sc_lv<32> ap_const_lv32_D;
    static const sc_lv<32> ap_const_lv32_1C;
    static const sc_lv<29> ap_const_lv29_1000;
    static const sc_lv<32> ap_const_lv32_2;
    // Thread declarations
    void thread_ap_clk_no_reset_();
    void thread_ap_done();
    void thread_ap_idle();
    void thread_ap_ready();
    void thread_ap_sig_bdd_169();
    void thread_ap_sig_bdd_21();
    void thread_ap_sig_bdd_403();
    void thread_ap_sig_cseq_ST_pp0_stg0_fsm_1();
    void thread_ap_sig_cseq_ST_st1_fsm_0();
    void thread_ap_sig_cseq_ST_st7_fsm_2();
    void thread_dct_coeff_table_0_address0();
    void thread_dct_coeff_table_0_ce0();
    void thread_dct_coeff_table_1_address0();
    void thread_dct_coeff_table_1_ce0();
    void thread_dct_coeff_table_2_address0();
    void thread_dct_coeff_table_2_ce0();
    void thread_dct_coeff_table_3_address0();
    void thread_dct_coeff_table_3_ce0();
    void thread_dct_coeff_table_4_address0();
    void thread_dct_coeff_table_4_ce0();
    void thread_dct_coeff_table_5_address0();
    void thread_dct_coeff_table_5_ce0();
    void thread_dct_coeff_table_6_address0();
    void thread_dct_coeff_table_6_ce0();
    void thread_dct_coeff_table_7_address0();
    void thread_dct_coeff_table_7_ce0();
    void thread_dst_address0();
    void thread_dst_ce0();
    void thread_dst_d0();
    void thread_dst_we0();
    void thread_exitcond1_fu_323_p2();
    void thread_grp_fu_455_p2();
    void thread_grp_fu_482_p1();
    void thread_grp_fu_482_p10();
    void thread_k_1_fu_329_p2();
    void thread_src1_address0();
    void thread_src1_ce0();
    void thread_src2_address0();
    void thread_src2_ce0();
    void thread_src3_address0();
    void thread_src3_ce0();
    void thread_src4_address0();
    void thread_src4_ce0();
    void thread_src5_address0();
    void thread_src5_ce0();
    void thread_src6_address0();
    void thread_src6_ce0();
    void thread_src7_address0();
    void thread_src7_ce0();
    void thread_src_address0();
    void thread_src_ce0();
    void thread_tmp1_fu_404_p2();
    void thread_tmp4_fu_408_p2();
    void thread_tmp_16_cast_fu_307_p1();
    void thread_tmp_17_cast_fu_428_p1();
    void thread_tmp_2_cast_fu_311_p1();
    void thread_tmp_4_fu_299_p3();
    void thread_tmp_5_fu_351_p2();
    void thread_tmp_cast_fu_347_p1();
    void thread_tmp_fu_335_p1();
    void thread_tmp_s_fu_412_p2();
    void thread_ap_NS_fsm();
};

}

using namespace ap_rtl;

#endif
